...
$(document).ready(function() {
  $.ajax({
    url: 'http://localhost:56795/BibliotecaService.svc/ListarTodosLivros',
    method: "GET",
    crossDomain: true,
    success: function(data) {
      exibirResultado('abc', data);
      $(data.ListarTodosLivrosResult).each(function(index, value) {
        var record = "<tr><td>" + value.CodLivro + "</td><td>" + value.TituloLivro + "</td><td>" + value.Autor + "</td><td>" +
        value.EditoraLivro + "</td><td>" + value.Situaçao +"</dt></tr>";
        $("Table").append(record);
      });
    },
    error: function(err) {
